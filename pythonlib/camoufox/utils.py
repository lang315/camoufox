...
def resolve_verstr(executable_path: Optional[Path] = None) -> str:
    ...
    Reads through _bundle_app_version_line() rather than opening the file here:
    on macOS the binary sits in Contents/MacOS and application.ini ships in
    Contents/Resources, so `Path(executable_path).parent / 'application.ini'`
    finds nothing on exactly the platform this repository builds for.
    """
    return _bundle_app_version_line(executable_path) or installed_verstr()
