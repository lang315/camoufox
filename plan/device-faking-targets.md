...
New `patches/css-media-spoofing.patch` hooks four `Gecko_MediaFeatures_*` getters in `layout/style/nsMediaFeatures.cpp` to answer from config: `ColorGamut` (was `dx->GetColorGamut()` → leaks P3/Rec2020 monitor), `VideoDynamicRange` (was `dx->GetScreenIsHDR()`, RFP-gated=off → leaks HDR), `PrefersColorScheme` (was forced dark globally via `ui.systemUsesDarkTheme`), and `PrefersReducedMotion` (RFP-gated=off → leaked host accessibility setting). Plain `MaskConfig::GetString`/`GetBool` on keys `cssMedia:colorGamut`/`dynamicRange`/`prefersColorScheme`/`prefersReducedMotion` (no new MaskConfig methods); `layout/style/moz.build` gets `LOCAL_INCLUDES += ["/camoucfg"]`. goapi defaults per-profile: srgb gamut, standard range, no reduced-motion, and a per-profile-random light/dark color-scheme (breaks the all-dark correlation). `DynamicRange` already returns Standard in FF150, so it's left alone. Guarded by `TestCSSMediaDefaults`; patch verified to apply under strict GNU patch 2.8 + BSD against pristine FF150. `go test ./...` green.
