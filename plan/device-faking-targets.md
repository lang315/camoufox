...
**Landed (CI-build-verify pending) — #20 screen.orientation + #23 touch trio:**
- **#20** — new `patches/screen-orientation-spoofing.patch` hooks `ScreenOrientation::GetType`/`GetAngle` (`dom/base/ScreenOrientation.cpp`, pristine — no other patch touches it) so `screen.orientation.type/angle` answer from `MaskConfig::GetString("screen:orientation")` / `GetUint32("screen:orientationAngle")` instead of the real host orientation (RFP off ⇒ they leaked `bc->GetCurrentOrientationType()`). No moz.build edit needed — `dom/base/moz.build` already gets `LOCAL_INCLUDES += ["/camoucfg"]` from `fingerprint-injection.patch`. goapi derives `screen:orientation` from the spoofed dims (landscape-primary when width ≥ height, else portrait-primary) with angle 0; guarded by `TestScreenOrientation`.
- **#23** — `settings/camoufox.cfg`: `defaultPref("dom.w3c_touch_events.enabled", 0)`. FF's default (2 = respect system) turns `'ontouchstart' in window` / `window.TouchEvent` on whenever the *host* has a touchscreen, contradicting the spoofed `maxTouchPoints=0` on a desktop profile. Forcing off keeps the touch trio coherent with camoufox's desktop presets (ponytail: a touch/mobile profile would need `1`).

`patches/screen-orientation-spoofing.patch` verified to apply under strict GNU patch 2.8 + BSD against pristine FF150. `go test ./...` green.
...
**Confirmed no-gap after investigation (2026-06-30):**
- **#29 Intl `resolvedOptions` — no change needed.** `calendar`/`numberingSystem`/number separators are not host-readable: ECMA-402/ICU derives them from the *resolved locale* via Firefox's bundled CLDR data, never the OS. The one real host-leak vector (a non-Gregorian/non-Latin OS surfacing `-u-ca-`/`-u-nu-` extensions) is already closed — `OSPreferences::GetRegionalPrefsLocales` returns the bare camou locale with no `-u-` extension; locale is bound at all three layers FF reads and timezone via `JS::SetRealmTimeZoneOverride`. Guarded at runtime by the harness `intl_consistency` subtest (calendar=gregory, numberingSystem=latn, timeZone=spoof).
- **#28 plugins/mimeTypes — no change needed.** Camoufox never touches the plugin code path (`nsPluginArray.cpp` appears only as unchanged moz.build context); it inherits stock FF150's 5 PDF pseudo-plugins + 2 mimeTypes verbatim, sharing one source of truth with `pdfViewerEnabled` so they structurally can't desync. Adding spoofing would be net-negative. (Cosmetic only: `settings/properties.json:52` `pdfViewerEnabled` has no C++ reader — a dead knob, left in place per surgical-change discipline.)

**Runtime verification harness (2026-06-30):** `goapi/runtime_spoof_test.go` launches a real Camoufox binary (gated on `CAMOUFOX_BIN`, skipped otherwise so `go test ./...` stays green), pins config via `WithConfig`+`WithNoFingerprint`, and asserts all 7 shipped spoofs take effect in page JS: webgpu-absent, webgl renderer/vendor, HEVC canPlayType/isTypeSupported/decodingInfo, CSS color-gamut/dynamic-range/prefers-color-scheme, screen.orientation, touch trio, voices lang, readPixels in-context determinism, plus the #29 Intl guard. Compile-verified (`go vet`); run with `CAMOUFOX_BIN=/path/to/camoufox go test ./goapi -run TestRuntimeSpoofs -v`.
...
**Open-target saturation audit (2026-06-30) — 8 targets vs real FF150 source, 0 ship:**
A parallel audit of the remaining open P1/P2 targets against the actual FF150.0.2 tree returned **zero new spoofs worth shipping** — the device-faking surface is saturated. Each was either already covered or not a real host/OS leak:
- **#24 resolution↔DPR MQ — DROP.** `Gecko_MediaFeatures_GetResolution` returns `pc->GetOverrideDPPX()` (nsMediaFeatures.cpp:200-201) before the real-DPR fall-through; camoufox sets `overrideDPPX` via Playwright `device_scale_factor`→Juggler (`TargetRegistry.js:653-654`; the pre-existing value is captured at `:472`). The `(resolution)` MQ already tracks the spoofed DPR. A C++ hook would add a third, divergent DPR source.
- **#26 CSS pointer/hover — DROP.** `patches/force-default-pointer.patch` already deletes the host `LookAndFeel::GetInt` read in `GetPointerCapabilities` and hardcodes `Fine|Hover` (desktop)/`Coarse` (Android). Coherent with #23. Nothing to add.
- **#11 keyboard layout — DROP (not a leak).** `navigator.keyboard`/`getLayoutMap` is Chromium-only, absent from FF150 (grep = 0 hits) → `undefined` is the genuine FF value. `KeyboardEvent.code` is physical/layout-independent; `.key` is active-only and driver-supplied under automation. No passive web-readable layout leak. Adding `getLayoutMap` would be a Chromium-on-Firefox bot tell.
- **#17 measureText/TextMetrics — DROP.** `anti-font-fingerprinting.patch` already perturbs every glyph advance in `gfxHarfBuzzShaper::ShapeText` per-profile (`fonts:spacing_seed`, default-on at generator.go:78-79) and threads `userContextId` through the canvas `MakeTextRun` path; remaining TextMetrics fields read from bundled per-OS fonts. The requested seed-rounding hook *is* the shipped implementation.
- **#21 battery — DROP (premise wrong).** FF150 **exposes** `navigator.getBattery` to content (coarsened); the plan/build-tester premise that it's removed/undefined is wrong for FF150 (confirmed by sannysoft run). The four value getters are already hooked by `fingerprint-injection.patch` (`battery:*`). Hiding the API would itself be a non-FF tell. (Residual nit: goapi doesn't *populate* `battery:*`, so a discharging profile can report level 1.0 — a minor coherence nicety, not a host/OS leak. Separately: build-tester `core.ts:518 noBatteryAPI` is wrong vs FF150 and should be fixed — a test fix, not a spoof.)
- **#22 Permissions API — DROP.** The `PermissionName` enum is a static WebIDL enum tied to Gecko **version**, not OS; camoufox is genuine Gecko 150 so the supported set already matches a real FF150. Grant/deny state owned by Playwright/juggler. Faking anything here would only create incoherence.
- **#18 StorageManager quota — DOWNGRADE (backlog, pref not patch).** Real uncovered leak (`navigator.storage.estimate().quota` ← free disk), but signal is noisy/disk-dependent (weak, mostly a private-mode detector), there's no canonical value (a fixed quota is a homogeneity tell; must keep quota≥usage), and FF150 already has the pref `dom.quotaManager.temporaryStorage.fixedLimit` (KB) — so the right fix is a per-profile randomized **pref written by goapi**, not a C++ MaskConfig hook. Low value.
- **#19 refresh-rate (rAF cadence) — DOWNGRADE (backlog, pref).** Not host/OS (monitor hardware). Already neutralized in both automation modes: Playwright `HeadlessCompositorWidget` software-vsync (0-playwright.patch:3381) and Xvfb virtual display → 60Hz. Only headed-on-real-high-refresh-monitor leaks. At most a one-line `layout.frame_rate=60` pref, which adds its own homogeneity tell.

Conclusion: no further C++ device-faking patches are warranted. Two low-value **pref-based** backlog items (#18 per-profile quota pref, #19 `layout.frame_rate`) remain optional. The shipped 7 C++/pref spoofs + existing camoufox patches cover the real host/OS-leaking surface.
